...
Outside the builder's own committed `DESIGN_BRIEF.md`, do not separately include, name, summarize, or expose any direction source file or path under `directions/`, `orthogonality-check.md`, either sibling candidate's `DESIGN_BRIEF.md`, the user-facing three-direction comparison table, or any other arena-level horizontal comparison in a builder dispatch. The builder may—and must—receive its assigned direction content inside its own complete brief; that brief is the sole permitted channel for the assigned direction. Do not include extra clues that enable a builder to infer or imitate sibling directions.
