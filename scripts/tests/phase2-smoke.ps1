...
$differentialScript = if ($DifferentialPath) { $DifferentialPath } else { Join-Path $PSScriptRoot 'controller-differential.ps1' }
...
try { $lifecycleOutput = @(& $lifecycleSmoke @parameters) } catch { throw "Lifecycle smoke test failed: $($_.Exception.Message)" }
$lifecycleResult = ConvertFrom-SmokeJson -Output $lifecycleOutput -Description 'Lifecycle smoke test' -RequirePass
try { $differentialOutput = @(& $differentialScript -BaseRoot $BaseRoot -PythonExecutable $PythonExecutable) } catch { throw "Controller differential failed: $($_.Exception.Message)" }
$differentialResult = ConvertFrom-SmokeJson -Output $differentialOutput -Description 'Controller differential' -RequirePass
[pscustomobject][ordered]@{
    status = 'PASS'; declarativeQa = $unitResult; lifecycle = $lifecycleResult; differential = $differentialResult
